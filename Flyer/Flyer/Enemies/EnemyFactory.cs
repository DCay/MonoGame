...
namespace Flyer.Factories
{
    ...
    {
        public static List<Enemy> Build(Texture2D texture, List<Enemy> list, Vector2 location,Texture2D projectileTexture)
        {
            for (int i = 0; i < 200; i++)
            {
                var nextDron = new Mine(texture,projectileTexture);
